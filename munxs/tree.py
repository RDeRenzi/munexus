...
import numpy as np
import munxs.napi as napi  # RDR
from munxs.napi import NeXusError  # RDR
...
class NeXusTree(napi.NeXus):
    ...
    def _readchildren(self,n):
        ...
        for _item in range(n):
            ...
            elif nxclass == b'SDS': # NXgetnextentry returns 'SDS' as the class for NXfields
            # RDR hack
                children[name] = self._readdata(name)
            else:
                print('opengroup: name {}, nxclass {}'.format(name,nxclass))
    ...
    def _readgroup(self):
        ...
        if 'target' in attrs and attrs['target'] != self.path:
            ...
            print('{} {} {} {}'.format(nxclass,name,attrs,children))
        ...
        if isinstance(value, NXattr):
            ...
            if isinstance(value, str):
                ...
                if self._data.size == 1:
                    if self._dtype[:5]=='bytes': # RDR
                        self._dtype='str' # RDR
                        self._data = self._data.astype('str') # RDR
                    # print('{} self._dtype = {}'.format(self._data,self._dtype))
...
class NXfield(NXobject):
    ...
    def __init__(self, value=None, name=b'field', dtype=None, shape=(), group=None,
                 attrs={}, **attr): # RDR
        if isinstance(value, list) or isinstance(value, tuple):
            value = np.array(value)
        self._value = value
        self._class = b'NXfield' # RDR
        self._name = name.replace(b' ',b'_') # RDR
        self._group = group
        self._dtype = dtype
        if dtype == 'char':
            self._dtype = 'char'
    ...
    def __init__(self, *items, **opts):
        ...
        if "entries" in opts.keys():
            for k,v in opts["entries"].items():
                print('entries k = {}, v = {}'.format(k,v))
                setattr(self, k.decode('utf-8'), v) # RDR
            del opts["entries"]
        self._attrs = AttrDict()
        ...
        for k,v in opts.items():
            setattr(self, k, v)
        if self.nxclass.decode('utf-8').startswith("NX"): # RDR
            if self.nxname == b"unknown": self._name = self.nxclass[2:]
...
#            return cmp(self.nxname, other.nxname)
    ...
    def insert(self, value, name=b'unknown'): # RDR
        """
        ...
        """
        if isinstance(value, NXobject):
            if name == b'unknown': name = value.nxname # RDR
    ...
    def __init__(self, signal=None, axes=None, *items, **opts):
        ...
        if signal is not None:
            if isinstance(signal,NXfield):
                if signal.nxname == b"unknown": signal.nxname = "signal"
            ...
            if axes is not None:
                ...
                for axis in axes:
                    ...
                    if isinstance(axis,NXfield):
                        if axis._name == b"unknown": axis._name = "axis%s" % i
